...
@section('content')
    <section class="section">
        ...
        <div class="container">
            <div class="row">
                <div class="col">
                    <h1 class="text-center">{{ __("textes.stripe_cancel_titre") }}</h1>
                    <div class="d-flex align-items-center flex-column">
                        <p class="w-75">{{ __("textes.stripe_cancel_description") }}</p>
                        <a class="btn btn-large my-4 btn-size text-white"
                           href="{{route("compte.index")}}">{{ __("textes.stripe_cancel_btn_compte") }}</a>
                        <a class="btn btn-large my-4 btn-size text-white"
                           href="{{route("accueil.index")}}">{{ __("textes.stripe_cancel_btn_compte_home") }}</a>
                    </div>
                </div>
            </div>
        </div>
